...
open class WSTagsField: UIScrollView {
    ...
    // For 1 line
    // Max number of lines of tags can display in WSTagsField before its contents become scrollable. Default value is 1, which means WSTagsField always resize to fit all tags.
    open var numberOfLines: Int = 1 {
        didSet {
            repositionViews()
        }
    }
    // For 2 line // uncomment it when requirement comes for 2 lines 
    // Max number of lines of tags can display in WSTagsField before its contents become scrollable. Default value is 2, which means WSTagsField always resize to fit all tags.
...
//    }
    ...
    open var tagCornerRadius: CGFloat = 12.0  // not for iPhone SE , just ignore SE as per Designer(Sujith Johns) & Manju

    open var cornerRadius: CGFloat = 12.0 { // not for iPhone SE , just ignore SE as per Designer(Sujith Johns) & Manju
        didSet {
    ...
    open var fieldTextContentType: UITextContentType! {
        set {
        //  textField.textContentType = fieldTextContentType /* This line is actual or original line from the library , commented by Ranjeet Raushan on 31st Dec 2019 , Don't remove future might reuse  */
            textField.textContentType = newValue /* This line is edited(changed) by Ranjeet Raushan on 31 Dec 2019  */
        }
        get {
    ...
    open var acceptTagOption: WSTagAcceptOption = .space
    ...
    open func selectTagView(_ tagView: WSTagView, animated: Bool = false) {
        ...
        //prasuna added this for remove tag
        guard let selectedIndex = tagViews.firstIndex(where: { $0.selected }) else {
        return
        }
        self.removeTagAtIndex(selectedIndex)
...
extension WSTagsField {
    ...
    fileprivate func enumerateItemRects(layoutWidth: CGFloat, using closure: (_ tagView: WSTagView?, _ tagRect: CGRect?, _ textFieldRect: CGRect?) -> Void) {
        ...
        curX += max(0, 6.0 - self.spaceBetweenTags)
        ...
        if textField.isEnabled {
            ...
            if availableWidthForTextField < 56.0 {
    ...
    private var maxHeightBasedOnNumberOfLines: CGFloat {
        ...
        return contentInset.top + contentInset.bottom + 25.0 * CGFloat(numberOfLines) + spaceBetweenLines * CGFloat(numberOfLines - 1)
    }
...
extension WSTagsField: UITextFieldDelegate {
    ...
    public func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        if acceptTagOption == .space && onShouldAcceptTag?(self) ?? true {

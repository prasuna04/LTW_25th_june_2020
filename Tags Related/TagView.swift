...
open class TagView: UIButton {
    ...
    /* written by Library(from here) - don't delete/change , future might reuse
    @IBInspectable open var textFont: UIFont = .systemFont(ofSize: 12) {
        didSet {
            titleLabel?.font = textFont
        }
    }
    written by Library(till here) - don't delete/change , future might reuse */
    
      /* written with customization by Ranjeet(from here) */
         @IBInspectable open var textFont: UIFont = .boldSystemFont(ofSize: 12) { // not for iPhone SE , just ignore SE as per Designer(Sujith Johns) & Manju
           didSet {
             titleLabel?.font =  UIFont.boldSystemFont(ofSize: 12)// not for iPhone SE , just ignore SE as per Designer(Sujith Johns) & Manju
           }
       }
      /* written with customization by Ranjeet(till here) */
    ...
    @IBInspectable open var removeButtonIconSize: CGFloat = 8 { // not for iPhone SE , just ignore SE as per Designer(Sujith Johns & Manju )
        didSet {
    ...
    @IBInspectable open var removeIconLineWidth: CGFloat = 2 { // not for iPhone SE , just ignore SE as per Designer(Sujith Johns & Manju )
        didSet {

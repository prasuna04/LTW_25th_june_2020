...
open class TagListView: UIView {
    ...
    @IBInspectable open dynamic var removeButtonIconSize: CGFloat = 8 { // not for iPhone SE , just ignore SE as per Designer(Sujith Johns)& Manju
        didSet {
            defer { rearrangeViews() }
            tagViews.forEach {
    ...
    @IBInspectable open dynamic var removeIconLineWidth: CGFloat = 2 { // not for iPhone SE , just ignore SE as per Designer(Sujith Johns) & Manju
        didSet {
            defer { rearrangeViews() }
            tagViews.forEach {
   ...
   written by Library(till here) - don't delete/change , future might reuse */
    ...
    /* written with customization by Ranjeet(from here) */
           @objc open dynamic var textFont: UIFont = .boldSystemFont(ofSize: 12) { // not for iPhone SE , just ignore SE as per Designer(Sujith Johns) & Manju
           didSet {
               defer { rearrangeViews() }
               tagViews.forEach {
                   $0.textFont = textFont
